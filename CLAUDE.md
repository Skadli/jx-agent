...
│   ├── L3 Identity   persona loader + 5s mtime watcher (5 md → system prompt)
...
- **system prompt assembly**: `engine/prompt_builder.py` — order is `persona` + `CLAUDE.md` + `<active_skills>` segment
...
- **persona swap**: edit any of `persona/{root,personality,beliefs,style,examples}.md`; watcher reloads within 5s
