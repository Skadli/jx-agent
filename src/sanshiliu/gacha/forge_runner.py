...
class ForgeRunner:
    ...
    async def _run_one_chapter(self, state: CardState, on_event: OnForgeEvent | None) -> None:
        ...
        self._evolve_persona(state, next_no, state.active_persona_chapter, coerced)
    ...
    def _write_biography(
        ...
        path = bio_dir / f"chapter-{chapter_no}.md"
        path.write_text("\n\n".join(body_parts) + "\n", encoding="utf-8")
        return path
    ...
    def _evolve_persona(
        ...
        best-effort：任一步失败只记日志、跳过演化，不影响传记 + 状态推进。
        不触碰 PersonaLoader——卡锻造不改本体当前人格（转生才改，PR3）。
        """
        try:
