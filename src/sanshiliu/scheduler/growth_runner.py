...
class GrowthRunner:
    ...
    def _build_prompt(self, state: GrowthState, chapter_no: int, age_range: str) -> str:
        ...
        lines: list[str] = [
            ...
            "**这一章要敢于天马行空、大胆发散**：别写成平淡的履历/流水账，让命运来个急转、丢进一段奇遇、"
            "开个大脑洞——穿越、修仙、异能、平行世界、撞见非凡的人或事都尽管上，越出常规越好，宁可惊人也不要无聊。"
            "只有两条底线：①真要写写实的经历（上学/工作/地名/年代）时，那部分得对得上现实、贴合本章年代"
            f"（约公历 {year_lo}-{year_hi} 年，真实的大学要真有那个专业、别造假机构）；"
            "②无论多放飞，故事内部要自洽、能圆回来、承接前文，不是凭空乱跳。",
            "",
        ]
        if state.chapters:
            lines.append(
