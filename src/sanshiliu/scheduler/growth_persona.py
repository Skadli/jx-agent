...
- **每个 chapter 目录至少一份非空 md**：否则 PersonaLoader 触发 ConfigError；靠"从上一章
  目录拷贝起步、只整段覆盖 LLM 这章演化的段落"保证连续 + 非空（缺的段落自然承接前章）。
- **协议/红线不被成长改写**：载体协议（≤60字 / <MSG> / 禁 markdown…）和安全红线写在
  persona/core/_protocol.md，**不在演化的 5 段之内**——被 _copy_core_md 每章原样带走、永不
  覆盖。人设 5 段因此可放心整段替换（长成校长就只剩校长），协议层照旧常驻。
...
def write_chapter_persona(
    *,
    ...
    """写第 N 章演化人格目录：先整盘拷上一章（连续 + 非空兜底），再整段覆盖 LLM 这章演化的段落。

    连续性 + 非空保证：从 prev_chapter_no 的人格目录拷贝起步，所以本章**没演化的段落自动
    承接前一章**；persona_sections 里给了的段落才整段覆写。即使 LLM 这章一个段落都没给，
    本章目录也等于上一章（仍非空），日常对话仍有完整人格。

    协议/红线为什么不会被冲掉：载体协议（≤60字、<MSG>、禁 markdown…）和安全红线放在
    persona/core/_protocol.md，**不在 PERSONA_SECTION_FILES 之内**——它被 _copy_core_md 每章
    原样带走、永不被 LLM 覆盖。人设 5 段因此可以整段替换（长成校长就只剩校长），协议层照旧常驻。
    ...
    # 1) 整盘拷上一章作基线（承接前文、保证非空，连 _protocol.md 等非演化文件一并带走）
    copied = _copy_core_md(prev, dst)
    # 2) 整段覆盖本章 LLM 演化的人设段落（协议/红线在 _protocol.md，不在这 5 段内、不受影响）
...
def make_active_core_provider(growth_state_path: Path, data_dir: Path) -> ActiveCoreProvider:
