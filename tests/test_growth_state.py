...
def test_advance_past_end_raises() -> None:
    ...
    with pytest.raises(ValueError, match="定格"):
        state.advance(ChapterRecord(age_range="30-31", summary="越界章"))
...
def test_delete_from_truncates_chapter_and_rewinds_age_and_pointer() -> None:
    state = _fresh_state()
    for i in range(4):
        state.advance(ChapterRecord(age_range=f"ch{i}", summary=f"第{i + 1}章"))
...
def test_delete_from_keeps_active_pointer_if_already_earlier() -> None:
    state = _fresh_state()
    for i in range(4):
        state.advance(ChapterRecord(age_range=f"ch{i}", summary=f"第{i + 1}章"))
    state.rollback(1)  # 激活指针先回到第 1 章
    state.delete_from(3)  # 删第 3、4 章；激活指针(1) 已 ≤ 新章数(2)，不动
    assert state.active_persona_chapter == 1
    assert state.current_chapter == 2


def test_delete_from_one_clears_all_back_to_start() -> None:
    state = _fresh_state()
    for i in range(3):
        state.advance(ChapterRecord(age_range=f"ch{i}", summary=f"第{i + 1}章"))
    removed = state.delete_from(1)  # 清空全部
...
def test_delete_from_out_of_range_raises() -> None:
    state = _fresh_state()
    state.advance(ChapterRecord(age_range="5-6", summary="第一章"))
    with pytest.raises(ValueError):
        state.delete_from(2)  # > current_chapter
    with pytest.raises(ValueError):
        state.delete_from(0)  # < 1
